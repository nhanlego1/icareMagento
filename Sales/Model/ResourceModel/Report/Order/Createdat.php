 ...
 *
 * @author      Nam PHam
 * @see         \Magento\Sales\Model\ResourceModel\Report\Order\Createdat
 */
    ...
    protected function _aggregateByField($aggregationField, $from, $to)
    {
        ...
        try {
            ...
            $select->from(
                ...
                []
            )->join(  // make sure that foreign-key constraints of user_id is met
                ['au' => $this->getTable('admin_user')], 
                'au.user_id = o.user_id',
                []
            )->where(
                'o.state NOT IN (?)',
            ...
            $select->group([$periodExpr, 'o.store_id', 'o.status', 'o.user_id']);
