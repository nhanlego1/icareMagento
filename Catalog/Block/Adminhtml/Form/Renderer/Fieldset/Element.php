 ...
 */
namespace Icare\Catalog\Block\Adminhtml\Form\Renderer\Fieldset;
...
class Element extends \Magento\Catalog\Block\Adminhtml\Form\Renderer\Fieldset\Element
{
     ...
     */
    protected $_template = 'Icare_Catalog::catalog/form/renderer/fieldset/element.phtml';
    ...
    {
        $element = $this->getElement();
        $disable = ['name', 'sku', 'quantity_and_stock_status'];
        $product = $this->getDataObject();
        if (in_array($element->getHtmlId(), $disable) && $product->getTypeId() != 'virtual') {
            return $this->getElement()->setDisabled(true)->getElementHtml();
        }else{
